...
CREATE TABLE ITEMS (
    ...
    ORDER_ID INTEGER NOT NULL -- OPTION 1
);
...
INSERT INTO ITEMS VALUES (7, 500,'BOTTLE', 5);
...
INSERT INTO ORDERS VALUES (5, 500, 2);
...
FROM CUSTOMERS C 
INNER JOIN ORDERS O ON C.ID = O.CUSTOMER_ID -- INTERSECT BY KEY "ON STATEMENT"
INNER JOIN ITEMS I ON I.ORDER_ID = O.ID
ORDER BY O.ID 
;

-- APPROACH 2: INNER JOIN -- ANOTHER SYNTAX
SELECT O.CUSTOMER_ID,
 C.NAME AS CUSTOMER_NAME,
O.ID AS ORDER_ID , 
O.AMOUNT AS ORDER_AMOUNT,
...
WHERE  O.CUSTOMER_ID = C.ID  -- INTERSECT BY KEY "ON STATEMENT"
AND I.ORDER_ID = O.ID 
ORDER BY O.ID 
;
